...
def train(loss_fn,experiment_folder,experiment_name,new_weights_path,train_img_split, train_msk_split,val_img_split, val_msk_split, preprocess_type = PREPROC_MATCH_CDF, first_layer_max_epochs=5):
    ...
    if TRAIN_RESUME and FINE_TUNE_NUM_OF_CHANNELS==1:
        ...
        first_layer_history = model.fit(
            mybatch_generator(list(zip(train_img_split, train_msk_split)), IN_ROWS, IN_COLS, BATCH_SIZE,num_of_channels=FINE_TUNE_NUM_OF_CHANNELS, max_possible_input_value=MAX_BIT, gen_type=GEN_TRAIN, preprocess_type=preprocess_type),
            steps_per_epoch=np.int32(np.ceil(len(train_img_split) / BATCH_SIZE)), epochs=first_layer_max_epochs, verbose=1,
            validation_data=mybatch_generator(list(zip(val_img_split, val_msk_split)), IN_ROWS, IN_COLS, BATCH_SIZE,num_of_channels=FINE_TUNE_NUM_OF_CHANNELS, max_possible_input_value=MAX_BIT, gen_type=GEN_VAL, shuffle=False, preprocess_type=preprocess_type),
    ...
    history = model.fit(
        ...
        validation_data=mybatch_generator(list(zip(val_img_split, val_msk_split)), IN_ROWS, IN_COLS, BATCH_SIZE,num_of_channels=FINE_TUNE_NUM_OF_CHANNELS, max_possible_input_value=MAX_BIT, gen_type=GEN_VAL, preprocess_type=preprocess_type),
        validation_steps=np.int32(np.ceil(len(val_img_split) / BATCH_SIZE)),
...
if __name__ == "__main__":
    ...
    try:
        train(loss_fn,experiment_folder,experiment_name,new_weights_path,train_img_split, train_msk_split,val_img_split, val_msk_split, preprocess_type=input_preprocess_type)
